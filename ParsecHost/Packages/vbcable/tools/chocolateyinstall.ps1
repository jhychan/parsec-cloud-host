...
$url        = 'https://download.vb-audio.com/Download_CABLE/VBCABLE_Driver_Pack43.zip' # download url, HTTPS preferred
$url64      = '' # 64bit URL here (HTTPS preferred) or remove - if installer contains both (very rare), use $url

# Download VB-cable installer zip
$downloadArgs = @{
  PackageName   = $env:ChocolateyPackageName
  FileFullPath  = Join-Path $env:Temp 'VBCABLE_Driver_Pack43.zip'
  url           = $url
  url64bit      = $url
...
# Extract vb-cable installer files
$unzipArgs = @{
  packageName   = $env:ChocolateyPackageName
  FileFullPath  = $zipPath
  Destination   = Join-Path $env:Temp 'vbcable-extract'
}
$unzipPath = Get-ChocolateyUnzip @unzipArgs

# Trust driver signature
$driverCertPath = Join-Path $unzipPath 'vbaudio_cable_win7.cer'
$driverCatPath = Join-Path $unzipPath 'vbaudio_cable_win7.cat'
$driverCat = Get-AuthenticodeSignature -FilePath $driverCatPath
$driverCat.SignerCertificate  | Export-Certificate -Type CERT -FilePath $driverCertPath
Import-Certificate -FilePath $driverCertPath -CertStoreLocation 'Cert:\LocalMachine\TrustedPublisher'
...
$packageArgs = @{
  ...
  file          = Join-Path $unzipPath 'VBCABLE_Setup.exe'
  file64        = Join-Path $unzipPath 'VBCABLE_Setup_x64.exe'
  #file         = $fileLocation

  softwareName  = 'VBCable*' #part or all of the Display Name as you see it in Programs and Features. It should be enough to be unique
  ...
  # MSI
  silentArgs    = "-i -h" # ALLUSERS=1 DISABLEDESKTOPSHORTCUT=1 ADDDESKTOPICON=0 ADDSTARTMENU=0
  validExitCodes= @(0)
  # OTHERS
